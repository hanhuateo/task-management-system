...
exports.promoteTask2Done = async (req, res, next) => {
	...
	try {
		...
		const [app_acronym] = await pool.query(
			"SELECT task_app_acronym FROM task WHERE task_id = ?",
			[task_id]
		);
		...
		const [app_permit_doing] = await pool.query(
			"SELECT app_permit_doing FROM application WHERE app_acronym = ?",
			[app_acronym[0].task_app_acronym]
		);
		...
		const [task_state] = await pool.query(
		...
		let [task_notes] = await pool.query(
		...
		await pool.execute(`UPDATE task SET task_notes = ? WHERE task_id = ?`, [
			task_notes,
			task_id,
		]);
